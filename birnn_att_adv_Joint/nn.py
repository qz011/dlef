...
def build_model(tparams, options):
    ...
    #####
    p_s0 = lstm_layer_0(tparams, input_state=tparams['Wemb'][xs0], mask=mask_xs0, options=options)
    p_s1 = lstm_layer_0(tparams, input_state=tparams['Wemb'][xs1], mask=mask_xs1, options=options)

    proj_s0 = tensor.concatenate((p_s0, p_s1), axis=1)
    pred_s0 = tensor.nnet.softmax(tensor.dot(proj_s0, tparams['Ws'])+tparams['bs'])

    f_s_pred_prob = theano.function(inputs=[xs0, xs1, mask_xs0, mask_xs1], 
                                    outputs=pred_s0.max(axis=1), name='f_s_pred_prob')

    f_s_pred = theano.function(inputs=[xs0, xs1, mask_xs0, mask_xs1],
                               outputs=pred_s0.argmax(axis=1), name='f_s_pred')

    s_cost = -tensor.mean(tensor.log(pred_s0[tensor.arange(ys0.shape[0]), ys0] + 1e-6))
    ...
    adv_pred_0 = tensor.nnet.softmax(tensor.dot(adv_proj_0, tparams['Ws'])+ tparams['bs']).flatten() 

    f_adv_pred_prob = theano.function(inputs=[x0, x1, mask0, mask1], 
                                    outputs=adv_pred_0.max(axis=0), name='f_adv_pred_prob')

    f_adv_pred = theano.function(inputs=[x0, x1,  mask0, mask1],
                               outputs=adv_pred_0.argmax(axis=0), name='f_adv_pred')
    ...
    ###
    adv_p_s0 = lstm_layer_0(tparams, input_state=tparams['p_Wemb'][xs0], mask=mask_xs0, options=options)
    adv_p_s1 = lstm_layer_0(tparams, input_state=tparams['p_Wemb'][xs1], mask=mask_xs1, options=options)

    proj_adv_s0 = tensor.concatenate((adv_p_s0, adv_p_s1), axis=1)
    pred_adv_s0 = tensor.nnet.softmax(tensor.dot(proj_s0, tparams['Ws'])+tparams['bs'])

    f_s_adv_pred_prob = theano.function(inputs=[xs0, xs1, mask_xs0, mask_xs1], 
                                    outputs=pred_adv_s0.max(axis=1), name='f_s_adv_pred_prob')

    f_s_adv_pred = theano.function(inputs=[xs0, xs1, mask_xs0, mask_xs1],
                               outputs=pred_adv_s0.argmax(axis=1), name='f_s_adv_pred')

    adv_s_cost = -tensor.mean(tensor.log(pred_adv_s0[tensor.arange(ys0.shape[0]), ys0] + 1e-6))
...
def output_pred_labels(options, f_pred, f_pred_prob, f_s_pred, f_s_pred_prob,prepare_data, data, iterator, verbose, path):
    f = open(path,'w')
    for _, test_index in iterator:
        x0, mask0, _ = prepare_data(data[0][test_index])
        x1, mask1, _ = prepare_data(data[1][test_index])

        xs0, mask_xs0, _ = prepare_data(data[3][test_index])
        xs1, mask_xs1, _ = prepare_data(data[4][test_index])
        
        pred_d_label = f_pred(x0, x1, mask0, mask1)
        pred_d_maxProb = f_pred_prob(x0, x1, mask0, mask1)

        pred_s_labels = f_s_pred(xs0, xs1, mask_xs0, mask_xs1)
        pred_s_maxProbs = f_s_pred_prob(xs0, xs1, mask_xs0, mask_xs1)

        f.write(str(pred_d_label)+' '+str(pred_d_maxProb)+'\n')

        for i in range(pred_s_labels.shape[0]):
            f.write(str(pred_s_labels[i])+' '+str(pred_s_maxProbs[i])+'\n')
...
def train_nn(
    ...
    #n_cueTypes = 4,
    n_vocb_words = 15375,  # Vocabulary size
    ...
    (x,
     ...
     f_adv_pred_prob, f_adv_pred, f_s_adv_pred_prob, f_s_adv_pred,
     d_cost, s_cost, adv_d_cost, adv_s_cost,
     d_cost_2, s_cost_2, cost) = build_model(tparams, model_options)
    ...
    f_adv_grad = theano.function(x+masks+y, adv_grads, name='f_adv_grad')
    ...
    try:
        for eidx in range(max_epochs):
            ...
            for train_batch_idx, train_index in kf:
                ...
                xs_0 = train[3][train_index]
                xs_1 = train[4][train_index]
                ys_0 = train[5][train_index]
            
                
                x_0, mask_0, _ = data.prepare_data(x_0)
                x_1, mask_1, _ = data.prepare_data(x_1)

                xs_0, mask_xs0, _ = data.prepare_data(xs_0)
                xs_1, mask_xs1, _ = data.prepare_data(xs_1)
                
                y_0 = numpy.asarray(y_0, dtype='int32')

                ys_0 = numpy.asarray(ys_0, dtype='int32')
                ys_0 = ys_0[:,1]
                ...
                cost = f_grad_shared(x_0, x_1, xs_0, xs_1, 
                                     mask_0, mask_1, mask_xs0, mask_xs1,
                                     y_0, ys_0)
                costs_list.append(cost)
                f_update(lrate)

                cur_adv_grad = f_adv_grad(x_0, x_1, xs_0, xs_1, 
                                          mask_0, mask_1, mask_xs0, mask_xs1,
                                          y_0, ys_0)
